...
@interface PSBottomDrawer ()<UIGestureRecognizerDelegate>
...
- (void)layoutSubviews {
    ...
    [self.drawerContainer ps_roundCorners:UIRectCornerTopLeft | UIRectCornerTopRight radius:24.f];
}
...
- (void)onDrag:(UIPanGestureRecognizer *)sender {
    ...
    if (scrollView) {
        if (scrollView.contentOffset.y > 0) {
            if (self.drawerContainer.y <= self.height - self.drawerContainer.height) {
                return;
            } else {
...
- (UIView *)drawerContainer {
    if (!_drawerContainer) {
        ...
        topLine.backgroundColor = UIColorFromRGB(0xD8D8D8);
