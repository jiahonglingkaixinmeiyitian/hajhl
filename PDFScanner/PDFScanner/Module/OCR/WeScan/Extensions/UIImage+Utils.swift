...
extension UIImage {
    ...
    func filter(imageFilter: GPUImageFilter) -> UIImage {
        if let gpuImagePicture = GPUImagePicture(image: self){
            gpuImagePicture.addTarget(imageFilter)
            imageFilter.useNextFrameForImageCapture()
            gpuImagePicture.processImage()
            return imageFilter.imageFromCurrentFramebuffer() ?? self
        }
        return self
    }
    
    func filterGroup(imageFilterGroup: GPUImageFilterGroup) -> UIImage {
        if let gpuImagePicture = GPUImagePicture(image: self){
            gpuImagePicture.addTarget(imageFilterGroup)
            imageFilterGroup.useNextFrameForImageCapture()
            gpuImagePicture.processImage()
            return imageFilterGroup.imageFromCurrentFramebuffer() ?? self
        }
        return self
    }
}
...
extension UIImage {
    ...
    static func generateFittingSizeForImageSize(_ imageSize: CGSize, inContainerSize containerSize: CGSize, aroundPadding padding: UIEdgeInsets) -> CGSize {
        
        let imageRatio = imageSize.width / imageSize.height
        let containerRatio = containerSize.width / containerSize.height // containerRatio is less than 1
        
        if imageRatio < 1 {
            // portrait.
            
            if imageRatio < containerRatio {
                let imageHeight = containerSize.height - padding.top - padding.bottom
                let imageWidth = imageHeight * imageRatio
                return CGSize(width: imageWidth, height: imageHeight)
            } else {
                let imageWidth = containerSize.width - padding.left - padding.right
                let imageHeight = imageWidth / imageRatio
                return CGSize(width: imageWidth, height: imageHeight)
            }
        } else {
            // landscape[No need to compare imageRatio & containerRatio]
