...
final class CaptureSessionManager: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    internal func capturePhoto() {
        ...
        var compressionRatio = 0.6
        if let settingModel = PSScannerSettingModel.allObjects().firstObject(){
            compressionRatio = Double(settingModel.cameraCompressionRatio())
        }
        let photoSettings = AVCapturePhotoSettings(format: [AVVideoCodecKey:AVVideoCodecType.jpeg, AVVideoCompressionPropertiesKey:[AVVideoQualityKey:compressionRatio]])//AVCapturePhotoSettings()
