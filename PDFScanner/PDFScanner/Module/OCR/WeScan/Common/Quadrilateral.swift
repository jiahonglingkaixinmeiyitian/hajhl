...
public struct Quadrilateral: Transformable {
    ...
    public var left: CGPoint {
        return CGPoint(x: bottomLeft.x + (topLeft.x - bottomLeft.x) / 2.0, y: bottomLeft.y + (topLeft.y - bottomLeft.y) / 2.0)
    }
    
    public var top: CGPoint {
        CGPoint(x: bottomLeft.x + (bottomRight.x - bottomLeft.x) / 2.0, y: bottomLeft.y + (bottomRight.y - bottomLeft.y) / 2.0)
    }
    
    public var bottom: CGPoint {
        return CGPoint(x: topLeft.x + (topRight.x - topLeft.x) / 2.0, y: topLeft.y + (topRight.y - topLeft.y) / 2.0)
    }
    
    public var right: CGPoint {
        return CGPoint(x: bottomRight.x + (topRight.x - bottomRight.x) / 2.0, y: bottomRight.y + (topRight.y - bottomRight.y) / 2.0)
    }
    
    // rotation
    public var leftRotateAngle: CGFloat {
        return atan(-(topLeft.x - bottomLeft.x) / (topLeft.y - bottomLeft.y))
    }
    
    public var topRotateAngle: CGFloat {
        return atan((bottomRight.y - bottomLeft.y) / (bottomRight.x - bottomLeft.x))
    }
    
    public var rightRotateAngle: CGFloat {
        return atan(-(topRight.x - bottomRight.x) / (topRight.y - bottomRight.y))
    }
    
    public var bottomRotateAngle: CGFloat {
        return atan((topRight.y - topLeft.y) / (topRight.x - topLeft.x))
    }
    ...
    func fixed() -> Quadrilateral {
        return Quadrilateral(topLeft: fiexedPoint(point: self.topLeft), topRight: fiexedPoint(point: self.topRight), bottomRight: fiexedPoint(point: self.bottomRight), bottomLeft: fiexedPoint(point: self.bottomLeft))
    }
    
    func fiexedPoint(point:CGPoint) -> CGPoint{
