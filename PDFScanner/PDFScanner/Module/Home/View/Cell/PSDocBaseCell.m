...
- (void)configWithModel:(PSFileModel *)model isEditMode:(BOOL)isEditMode {
    ...
    if (model.isDir) {
        ...
        self.contentCountLabel.text = @(model.imageDatas.count).stringValue;
        self.fileFirstImageView.hidden = NO;
        if (model.imageDatas.firstObject) {
            self.fileFirstImageView.image = [UIImage imageWithData:model.imageDatas.firstObject];
        }
    }
}
