...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self animateShutterButton:NO completionBlock:nil];
}

- (void)animateShutterButton:(BOOL)hidden completionBlock:(void (^)(BOOL finished))completionBlock {
    
    [UIView animateWithDuration:hidden ? 0 : 0.3 delay:0 usingSpringWithDamping:0.6 initialSpringVelocity:0.1 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            //
            self.scanButton.alpha = hidden ? 0 : 1;
            self.scanButton.transform = hidden ? CGAffineTransformMakeScale(0.01, 0.01) : CGAffineTransformMakeScale(1, 1);
...
- (void)_jumpToCameraScanner {
    
    dispatch_async(dispatch_get_main_queue(), ^{
        
        [self animateShutterButton:YES completionBlock:^(BOOL finished) {
...
- (void)_showCameraDeniedAlert {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        //
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Tips" message:@"Camera access is forbidden, please allow camera access in Setting" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"Ignore" style:UIAlertActionStyleCancel handler:nil];
        UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"Go" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)_shareWithIndex:(NSInteger)index {
    ...
    if (index == 0) {
        ...
        for (PSFileModel *file in selectedFiles) {
            NSInteger i = 1;
            for (PSPictureModel *picture in file.pictures) {
                [items addObject:picture.imageData];
        ...
        // pdf
        for (PSFileModel *file in selectedFiles) {
            NSData *pdfData = [PSPDFUtil generatePDFDataWithImageDatas:(NSArray *)file.pictures];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.isDir) {
        ...
        for (PSPictureModel *picture in model.pictures) {
            [arr addObject:[UIImage imageWithData:picture.imageData]];
